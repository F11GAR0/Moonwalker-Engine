...
enum PacketEnumeration
{
	...
	ID_VEHICLE_SYNC = 200,
	ID_AIM_SYNC = 203,
	ID_STATS_UPDATE = 205, //fixed by F11GAR0
	ID_BULLET_SYNC = 206,
	ID_PLAYER_SYNC = 207,
	ID_MARKERS_SYNC,
	ID_UNOCCUPIED_SYNC,
	...
	ID_RCON_RESPONCE,
	ID_WEAPONS_UPDATE,
	ID_ECTROPIC_SYNC, //i dont fucking know what is this, there are was STATS_UPDATE but STATS id is 205
 	ID_GOC_SYNC = 1337,
	ID_BE_SYNC
};
...
typedef unsigned short LQ_PlayerIndex;
...
struct LQ_PlayerID
{
	...
	LQ_PlayerID& operator = (const LQ_PlayerID& input)
	{
	...
	bool operator==(const LQ_PlayerID& right) const;
	bool operator!=(const LQ_PlayerID& right) const;
	bool operator > (const LQ_PlayerID& right) const;
	bool operator < (const LQ_PlayerID& right) const;
};
...
struct LQ_RPCNode
{
	...
	void(*staticFunctionPointer) (LQ_RPCParameters *rpcParms);
};

/// Store Statistics information related to network usage 
struct LQ_RakNetStatisticsStruct
{
...
class LQ_RakClientInterface
{
public:
	virtual ~LQ_RakClientInterface() {};
	...
	virtual int GetPlayerPing(const LQ_PlayerID playerId);
	...
	virtual void RegisterAsRemoteProcedureCall(int* uniqueID, void(*functionPointer) (LQ_RPCParameters *rpcParms));
	...
	virtual bool RPC_(int* uniqueID, BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp, LQ_NetworkID networkID);
	...
	virtual BitStream * GetStaticClientData(const LQ_PlayerID playerId);
	virtual void SetStaticClientData(const LQ_PlayerID playerId, const char *data, const int length);
	virtual void SendStaticClientDataToServer(void);
	virtual LQ_PlayerID GetServerID(void) const;
	virtual LQ_PlayerID GetPlayerID(void) const;
	virtual LQ_PlayerID GetInternalID(void) const;
	virtual const char* PlayerIDToDottedIP(const LQ_PlayerID playerId) const;
	...
	virtual LQ_RakNetStatisticsStruct * const GetStatistics(void);
	...
	virtual LQ_PlayerIndex GetPlayerIndex(void);
};
