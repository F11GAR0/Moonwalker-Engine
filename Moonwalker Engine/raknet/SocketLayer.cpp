...
#ifdef PROXY
		sa.sin_addr.S_un.S_addr = rakPeer->proxy_client->dest.sin_addr.S_un.S_addr;
		portnum = g_SAMP->ulPort;

		rakPeer->proxy_client->recv_tick = clock() + 1000;
		len -= 10;
#else
		portnum = ntohs( sa.sin_port );
#endif
#ifdef PROXY
		ProcessNetworkPacket( sa.sin_addr.s_addr, portnum, &data[10], len, rakPeer );
#else
...
#endif
int SocketLayer::SendTo( SOCKET s, const char *data, int length, unsigned int binaryAddress, unsigned short port, RakPeer *peer)
{
#ifdef PROXY
	int tick = clock();
	if (tick > peer->proxy_client->recv_tick) {
		if (peer->proxy_client->connected = proxy_connect(peer->proxy_client, 0))
			peer->proxy_client->recv_tick = tick + 1000;
		else
			return 0;
	}
#endif
...
#ifdef PROXY
	enc = &encr[10];
	sa.sin_addr.S_un.S_addr = peer->proxy_client->dst_addr;
	sa.sin_port = peer->proxy_client->dst_port;
		
	*(WORD*)packet = 0x00;
	packet[2] = 0x00;
	packet[3] = 0x01;
	*(unsigned int*)&packet[4] = peer->proxy_client->dest.sin_addr.S_un.S_addr;
	*(unsigned short*)&packet[8] = peer->proxy_client->dest.sin_port;
#else
	enc = encr;
...
#endif
	sa.sin_family = AF_INET;
	kyretardizeDatagram((unsigned char *)data, length, enc, port);
...
#ifdef PROXY
		len = sendto( s, (char*)packet, length + 1, 0, ( const sockaddr* ) & sa, sizeof( struct sockaddr_in ) );
#else
		len = sendto( s, (char*)packet, length + 1, 0, ( const sockaddr* ) & sa, sizeof( struct sockaddr_in ) );
#endif
	}
...
int SocketLayer::SendTo( SOCKET s, const char *data, int length, char ip[ 16 ], unsigned short port, RakPeer *peer )
{
	unsigned int binaryAddress;
	binaryAddress = inet_addr( ip );
	return SendTo( s, data, length, binaryAddress, port, peer );
}
