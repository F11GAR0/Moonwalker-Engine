...
{
#ifdef PROXY
	proxy_client_remove(proxy_client);
#endif
//	unsigned i;
...
{
#ifdef PROXY
	proxy_client->dest.sin_addr.S_un.S_addr = inet_addr(host);
	proxy_client->dest.sin_port = htons(remotePort);
#endif
...
bool RakPeer::RPC( int* uniqueID, const char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp)
{
	if ( *uniqueID > 256 )
		return false;
	...
	Send(&outgoingBitStream, priority, reliability, orderingChannel, remoteSystemList[0].playerId, false);
	return true;	
}

bool RakPeer::RPC( int* uniqueID, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp)
{
	if (bitStream)
		return RPC(uniqueID, (const char*) bitStream->GetData(), bitStream->GetNumberOfBitsUsed(), priority, reliability, orderingChannel, shiftTimestamp);
	else
		return RPC(uniqueID, 0,0, priority, reliability, orderingChannel, shiftTimestamp);
}

bool RakPeer::RPC_( int* uniqueID, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp, NetworkID networkID)
{
	return 0;
}
...
{
#ifdef LOG
	Log("RakPeer::SendImmediate %d", (unsigned char)data[0]);
#endif
	unsigned *sendList;
	...
	for (sendListIndex=0; sendListIndex < sendListSize; sendListIndex++)
	{
		...
		{
#ifdef LOG
			Log("else reliabilityLayer.Send");
#endif
